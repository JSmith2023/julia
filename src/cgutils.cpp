...
static void find_perm_offsets(jl_datatype_t *typ, SmallVectorImpl<unsigned> &res, unsigned offset)
{
    ...
    if (!typ->layout || typ->layout->npointers == 0)
        return;
    jl_svec_t *ftypes = jl_get_fieldtypes(typ);
    JL_GC_PUSH1(&ftypes);
    size_t i, ntypes = jl_svec_len(ftypes);
    for (i = 0; i < ntypes; i++) {
        jl_value_t *_fld = jl_svecref(ftypes, i);
...
Type *_julia_struct_to_llvm(jl_codegen_params_t *ctx, LLVMContext &ctxt, jl_value_t *jt, bool *isboxed);
